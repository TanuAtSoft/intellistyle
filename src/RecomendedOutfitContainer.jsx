...
const RecomendedOutfitContainer = ({ completeOutlook }) => {
  const [mainProduct, setMainProduct] = React.useState();
  const [completeTheLookArr, setCompleteTheLookArr] = React.useState();
  const [onTopRight, setOnTopRight] = React.useState();
  const [onTopLeft, setOnTopLeft] = React.useState();
  const [onBottomLeft, setOnBottomLeft] = React.useState();
  const [footwearBottomRight, setFootwearBottomRight] = React.useState();

  React.useEffect(() => {
    if (completeOutlook && completeOutlook.garments) {
      const product = fetchMainProduct(completeOutlook?.garments);
      setMainProduct(product);
      const footwear = getCatProducts(completeOutlook?.garments,"Footwear")
      setFootwearBottomRight(footwear)
      const tempArr = completeOutlook?.garments.filter(
        (g) =>
          g.remote_product_id !== product[0].remote_product_id &&
          g.remote_product_id !== footwear[0]?.remote_product_id
      );
      setFootwearBottomRight(footwear);
      ...
      if (onTopLeftProduct.length === 0) {
        for (let i = 0; i < completeTheLookArr.length; i++) {
          if (
            ...
            completeTheLookArr[i].remote_product_categories[0] !==
            footwearBottomRight[0]?.remote_product_categories[0]
          ) {
...
export default RecomendedOutfitContainer;
